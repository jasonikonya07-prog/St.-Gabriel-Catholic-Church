...
const accountTypes = new Set(Object.keys(refreshCookieNames));

function assertAccountType(accountType) {
  if (!accountTypes.has(accountType)) {
    throw new Error("A valid account type is required for authentication tokens.");
  }
}
...
export function generateToken(account, accountType) {
  return signAccessToken(account, accountType);
}
